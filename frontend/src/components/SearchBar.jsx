  ...
  const handleSearch = async (searchQuery = query, currentFilters = filters) => {
    ...
    try {
      const searchParams = {
        ...
        ...currentFilters,
        calendarIds: currentFilters.calendarIds || selectedCalendars.join(',')
      };

      // Remove 'all' values
      if (searchParams.status === 'all') delete searchParams.status;
      if (searchParams.eventType === 'all') delete searchParams.eventType;
      if (!searchParams.startDate) delete searchParams.startDate;
      if (!searchParams.endDate) delete searchParams.endDate;

      const events = await notificationsAPI.searchEvents(searchParams);
      setResults(events);
      setShowResults(true);
    } catch (error) {
